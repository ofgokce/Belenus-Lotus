...
class MoviesViewController: UIViewController, Instantiatable {
    ...
    private func update(withResult result: Result<[Movie], Error>) {
        DispatchQueue.main.async { [weak self] in
            switch result {
            case let .success(movies):
                self?.viewModel.update(movies) {
                    self?.collectionView.reloadData()
                }
            case let .failure(error):
                self?.viewModel.update([] as [Movie])
    ...
    private func updateGenreButton() {
        DispatchQueue.main.async { [weak self] in
            ...
            self?.genreButton.setNeedsLayout()
        }
    }
...
extension MoviesViewController: UICollectionViewDataSource, UICollectionViewDelegate, UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        navigationController?.pushViewController(MovieDetailViewController.create(withId: viewModel.cellData[indexPath.row].id), animated: true)
    }
